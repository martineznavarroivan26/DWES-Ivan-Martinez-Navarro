<?php
    ...
    class FormController{
        ...
        public function nuevoParticipanteForms(){
            ...
            header('Location: ./index.php?action=forms');
            exit;
        }
        ...
        public function buscarParticipante() {
            ...
            if ($cod <= 0) {
                header('Location: ./index.php?action=forms&error_busqueda=1');
                exit;
            }

            $participanteModel = new ParticipanteModel();
            $fila = $participanteModel->obtenerParticipante($cod);

            if ($fila) {
                // Participante existe: cargarlo en sesion
                $_SESSION['cod_participante_actual'] = $cod;
                $_SESSION['formularios_rellenados'] = [];
                header('Location: ./index.php?action=forms&participante_set=1');
            } else {
                // No existe: ir al formulario de creacion con el codigo prelleno
                header('Location: ./index.php?action=forms&need_participante=1&cod_nuevo=' . $cod);
            }
            exit;
        }
