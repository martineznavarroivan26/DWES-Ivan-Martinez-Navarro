...
<body>
    ...
    <?php
        $codNuevo = isset($_GET['cod_nuevo']) ? (int) $_GET['cod_nuevo'] : 0;
        $errorBusqueda = isset($_GET['error_busqueda']) && $_GET['error_busqueda'] === '1';
    ?>

    <?php if (!$hayParticipanteActivo && !$needParticipante): ?>
        <section>
            <h2>Buscar participante</h2>
            <?php if ($errorBusqueda): ?>
                <p><strong>Introduce un codigo de participante valido.</strong></p>
            <?php endif; ?>
            <form action="./index.php" method="POST">
                <input type="hidden" name="action" value="buscarParticipante">
                <label for="cod_buscar">Codigo de participante</label>
                <input id="cod_buscar" name="cod_participante" type="number" min="1" required>
    ...
    <?php if (!$hayParticipanteActivo && $needParticipante): ?>
        <section>
            <h2>Participante <?php echo $codNuevo > 0 ? $codNuevo : ''; ?> no encontrado — Registrar nuevo participante</h2>
            <form action="./index.php" method="POST">
                <input type="hidden" name="action" value="insertarParticipante">
                <input type="hidden" name="origen" value="forms">
                <input type="hidden" name="cod_participante" value="<?php echo $codNuevo > 0 ? $codNuevo : ''; ?>">
    ...
    <?php if ($hayParticipanteActivo): ?>
        ...
        <div class="forms-cloud">
            <a href="./index.php?action=forms&form=actividad" id="btnActi" class="cloud-link <?php echo ($formularioSeleccionado === 'actividad') ? 'is-active' : ''; ?>">Actividad fisica<?= etiquetaRellenado('actividad', $formulariosRellenados) ?></a>
            <a href="./index.php?action=forms&form=alimentacion" id="btnAlim" class="cloud-link <?php echo ($formularioSeleccionado === 'alimentacion') ? 'is-active' : ''; ?>">Alimentacion<?= etiquetaRellenado('alimentacion', $formulariosRellenados) ?></a>
            <a href="./index.php?action=forms&form=sueno" id="btnSueno" class="cloud-link <?php echo ($formularioSeleccionado === 'sueno') ? 'is-active' : ''; ?>">Habitos de sueño<?= etiquetaRellenado('sueno', $formulariosRellenados) ?></a>
            <a href="./index.php?action=forms&form=antropometrico" id="btnAntro" class="cloud-link <?php echo ($formularioSeleccionado === 'antropometrico') ? 'is-active' : ''; ?>">Datos antropometricos<?= etiquetaRellenado('antropometrico', $formulariosRellenados) ?></a>
        </div>
    ...
    <?php if ($formularioRellenado): ?>
        <p><strong>Este formulario ya fue rellenado. Puedes volver a enviarlo para corregir errores.</strong></p>
    <?php endif; ?>

    <?php if ($formulariosCompletos): ?>
        <p><strong>Has completado todos los formularios de este participante.</strong></p>
        <div class="next-participant-cloud">
            <form action="./index.php" method="POST" class="next-participant-form">
    ...
    <section id="actividad">
        ...
        <form action="./index.php" method="POST">
            ...
            <label>
                <input type="checkbox" class="ns-check" data-target="AcF2">
            ...
            <label>
                <input type="checkbox" class="ns-check" data-target="AcF4">
                No sabe/no esta seguro
            </label><br>
            ...
            <label>
                <input type="checkbox" class="ns-check" data-target="AcF6">
                No sabe/no esta seguro
            </label><br>
            ...
            <label>
                <input type="checkbox" class="ns-check" data-target="AcF7">
                No sabe/no esta seguro
            </label><br>
    ...
    <script>
    (function() {
        function getNoSabeHiddenId(inputId) {
            return inputId + '_nosabe_hidden';
        }

        function ensureNoSabeHidden(input) {
            var hiddenId = getNoSabeHiddenId(input.id);
        ...
        function removeNoSabeHidden(input) {
            var hidden = document.getElementById(getNoSabeHiddenId(input.id));
            if (hidden) {
                hidden.remove();
            }
        }

        function setNoSabeState(check) {
            var targetId = check.getAttribute('data-target');
            ...
            if (check.checked) {
                input.value = '';
                input.disabled = true;
                ensureNoSabeHidden(input);
            } else {
                input.disabled = false;
                removeNoSabeHidden(input);
            }
        }

        function disableQuestionInput(inputId) {
            ...
            var checks = document.querySelectorAll('#actividad .ns-check[data-target="' + inputId + '"]');
        ...
        function enableQuestionInput(inputId) {
            var input = document.getElementById(inputId);
            if (!input) {
                return;
            }
        ...
        function applySkipRule(triggerId, blockedId) {
            var trigger = document.getElementById(triggerId);
            if (!trigger) {
                return;
            }

            if (parseInt(trigger.value, 10) === 0) {
                disableQuestionInput(blockedId);
            } else {
                enableQuestionInput(blockedId);
            }
        }

        window.actualizarActividadDependencias = function() {
            var checks = document.querySelectorAll('#actividad .ns-check');
            checks.forEach(function(check) {
                setNoSabeState(check);
            });

            applySkipRule('AcF1', 'AcF2');
            applySkipRule('AcF3', 'AcF4');
            applySkipRule('AcF5', 'AcF6');
        };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            ['AcF1', 'AcF3', 'AcF5'].forEach(function(id) {
            ...
            window.actualizarActividadDependencias();
        });
    })();
    </script>
    <?php endif; ?>
    ...
    <section id="antropometrico">
        ...
        <form action="./index.php" method="POST">
            ...
            <input id="Ant1" name="Ant1" type="number" step="0.01" required oninput="calcularAntropometrico()"><br>
            <label for="Ant2">Ant2 - Talla (cm)</label>
            <input id="Ant2" name="Ant2" type="number" step="0.01" required oninput="calcularAntropometrico()"><br>
            <label for="Ant3">Ant3 - IMC (autocalculado)</label>
            <input id="Ant3" name="Ant3" type="number" step="0.01" required readonly><br>

            <label for="Ant4">Ant4 - Clasificacion IMC (autocalculada)</label>
            ...
            <input id="Ant5" name="Ant5" type="number" step="0.01" required oninput="calcularAntropometrico()"><br>
            <label for="Ant6">Ant6 - Cadera (cm)</label>
            <input id="Ant6" name="Ant6" type="number" step="0.01" required oninput="calcularAntropometrico()"><br>
            <label for="Ant7">Ant7 - ICC (autocalculado)</label>
            <input id="Ant7" name="Ant7" type="number" step="0.0001" required readonly><br>
            <label for="Ant8">Ant8 - Indice cintura-altura (autocalculado)</label>
            <input id="Ant8" name="Ant8" type="number" step="0.0001" required readonly><br>

            <script>
            function calcularAntropometrico() {
                var peso = parseFloat(document.getElementById('Ant1').value);
                var tallaCm = parseFloat(document.getElementById('Ant2').value);
                var cintura = parseFloat(document.getElementById('Ant5').value);
                var cadera = parseFloat(document.getElementById('Ant6').value);

                if (peso > 0 && tallaCm > 0) {
                    var tallaM = tallaCm / 100;
                    var imc = peso / (tallaM * tallaM);
                    document.getElementById('Ant3').value = imc.toFixed(2);

                    var selectAnt4 = document.getElementById('Ant4');
                    if (imc < 18.5) {
                        selectAnt4.value = 'Bajo peso';
                    } else if (imc < 25) {
                        selectAnt4.value = 'Normopeso';
                    } else if (imc < 30) {
                        selectAnt4.value = 'Sobrepeso';
                    } else {
                        selectAnt4.value = 'Obesidad';
                    }

                    if (cintura > 0) {
                        document.getElementById('Ant8').value = (cintura / tallaCm).toFixed(4);
                    }
                }

                if (cintura > 0 && cadera > 0) {
                    document.getElementById('Ant7').value = (cintura / cadera).toFixed(4);
                }
            }
            </script>
    ...
    <script>
    (function() {
        var form = '<?= htmlspecialchars($formularioSeleccionado, ENT_QUOTES, 'UTF-8') ?>';
        var cod  = '<?= $codParticipanteActivo ?>';
        var key  = 'borrador_' + form + '_' + cod;

        document.addEventListener('DOMContentLoaded', function() {
            var seccion = document.querySelector('section#' + form);
            if (!seccion) return;
            var formEl = seccion.querySelector('form');
            if (!formEl) return;
            ...
            if (guardado) {
                try {
                    ...
                    Object.keys(datos).forEach(function(nombre) {
                        ...
                        var campos = formEl.querySelectorAll('[name="' + nombre + '"]');
                        campos.forEach(function(campo) {
                            ...
                            } else if (valor === '-1' && campo.id) {
                                var checkNoSabe = formEl.querySelector('.ns-check[data-target="' + campo.id + '"]');
                                if (checkNoSabe) {
                                    checkNoSabe.checked = true;
                                }
                    ...
                    if (typeof window.actualizarActividadDependencias === 'function') {
                        window.actualizarActividadDependencias();
                    }

                    // Recalcular campos automaticos del formulario antropometrico
                    if (typeof calcularAntropometrico === 'function') {
                        calcularAntropometrico();
                    }
                } catch(e) {
            ...
            // Guardar borrador al modificar cualquier campo
            function guardarBorrador() {
                var datos = {};
                formEl.querySelectorAll('input, select, textarea').forEach(function(campo) {
                    if (!campo.name) return;
                    if (campo.type === 'hidden') return;

                    if (campo.type === 'radio' || campo.type === 'checkbox') {
                        if (campo.checked) datos[campo.name] = campo.value;
                    } else if (campo.disabled && campo.id) {
                        var checkNoSabe = formEl.querySelector('.ns-check[data-target="' + campo.id + '"]');
                        if (checkNoSabe && checkNoSabe.checked) {
                            datos[campo.name] = '-1';
                        }
                    } else {
            ...
            formEl.addEventListener('input', guardarBorrador);
            formEl.addEventListener('change', guardarBorrador);
        });
    })();
    </script>
